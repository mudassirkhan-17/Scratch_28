...
# Core imports
from src.helpers.inputs import *
from src.helpers.indicators import calculate_indicator
from src.helpers.comparision_types import *
from src.helpers.config_loader import load_json_config
from src.helpers.metrics import calculate_advanced_metrics
import src.helpers.comparisons as comp
from src.llm.llm_strategy_parser import StrategyParser
...
def execute_strategy():
    ...
    print("1. 🤖 Natural Language (AI-powered)")
    print("2. 📂 Load from JSON config")
    print("3. 💬 Interactive (manual input)")
    
    input_mode = input("\nEnter choice (1-3) [default: 1]: ").strip() or "1"
    
    if input_mode == "1":
        # NATURAL LANGUAGE MODE: AI-powered parsing
        print("\n" + "="*70)
        print("🤖 AI STRATEGY PARSER")
        print("="*70)
        print("Describe your trading strategy in plain English!")
        print("\nExamples:")
        print("  • 'Backtest AAPL with 20-day SMA crossing above 50-day SMA'")
        print("  • 'Short TSLA when RSI goes above 70, exit when below 30'")
        print("  • 'Multi-ticker: AAPL 60%, MSFT 40%, use EMA crossover'")
        print("="*70)
        print()
        
        strategy_description = input("📝 Describe your strategy: ").strip()
        ...
        # JSON MODE: Load from existing file
        use_json = 'y'
    else:
    ...
    if use_json == 'y':
        ...
        # filename is already set if from AI mode (input_mode == "1")
        if input_mode != "1":
    ...
    if strategy_complexity == "single":
        ...
        if use_json != 'y':
            ...
            filename = "results/input/config_multi_ticker.json"
        ...
        if use_json != 'y':
            # Create input folder if it doesn't exist
            os.makedirs("results/input", exist_ok=True)
            filename = "results/input/config_multi_ticker_multi.json"
