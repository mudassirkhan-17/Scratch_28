...
class PortfolioManager:
    ...
    def __init__(self, initial_cash=1000, per_trade_config=None):
        ...
        self.initial_cash = validate_positive_number(initial_cash, "Initial cash", 1000)
        self.original_capital = self.initial_cash       # Store original starting capital (never changes)
        
        # Per-trade allocation setup
        if per_trade_config:
            self.per_trade_config = per_trade_config  # Store original config
            self.invested_amount = validate_positive_number(
                per_trade_config.get('amount_per_trade', initial_cash), 
                "Amount per trade", 
                initial_cash
            )
            self.allocation_percentage = validate_positive_number(
                per_trade_config.get('percentage', 100), 
                "Allocation percentage", 
                100
            )
        else:
            self.per_trade_config = None
            self.invested_amount = self.initial_cash * 1.0  # Fallback: 100% (old behavior)
